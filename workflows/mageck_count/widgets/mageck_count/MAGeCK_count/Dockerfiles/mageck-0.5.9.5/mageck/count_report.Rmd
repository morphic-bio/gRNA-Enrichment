...
```{r}
cstable=read.table(count_summary_file,header = T,as.is = T, sep = '\t')
nc_table=read.table(normalized_cnt_file,header = T,as.is = T, sep = '\t')
...
genboxplot<-function(filename,isfile=T,...){
  #slmed=read.table(filename,header=T, sep = '\t')
  if(isfile){
    slmed=read.table(filename,header=T, sep = '\t')
  }else{
    slmed=filename;
